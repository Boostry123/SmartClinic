...
import { Home, Calendar, Users, Activity, Sparkles, Bell, Search, LogOut, Menu, X } from 'lucide-react';
        ...
        <nav className="flex items-center justify-between w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16">
          ...
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => navigate('/')}>
            ...
            <div className="flex flex-col">
              <span className="font-bold text-lg text-white tracking-tight">SmartClinic</span>
              <span className="text-[9px] font-bold text-indigo-300 tracking-[0.15em] opacity-80 uppercase">AI SYSTEM</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
              ...
              <button className="hidden sm:flex p-2.5 text-indigo-300 hover:text-white hover:bg-white/5 rounded-full transition-colors" aria-label="Search">
                  <Search size={18} />
              </button>
              
              {/* Notification Bell (No red dot by default) */}
              <button className="hidden sm:flex p-2.5 text-indigo-300 hover:text-white hover:bg-white/5 rounded-full transition-colors relative" aria-label="Notifications">
                  <Bell size={18} />
